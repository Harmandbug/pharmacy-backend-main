...
function get_users_by_manager_id($connect) {
    $query = "SELECT du_user FROM managers_users WHERE du_manager = '".$_SESSION['manager_id']."'";
...
function get_community_posts($connect)
{
    
    $sentence = $connect->prepare("SELECT community_posts.*,tags.tag_title AS tag_title FROM community_posts,tags WHERE community_posts.post_tag = tags.tag_id ORDER BY post_id DESC"); 
    $sentence->execute();
    return $sentence->fetchAll();
}
...
function get_community_post_per_id($connect, $id_post){
    $sentence = $connect->query("SELECT community_posts.*,tags.tag_title AS tag_title FROM community_posts,tags WHERE post_id = $id_post AND community_posts.post_tag = tags.tag_id LIMIT 1");
    $sentence = $sentence->fetchAll();
    return ($sentence) ? $sentence : false;
}
